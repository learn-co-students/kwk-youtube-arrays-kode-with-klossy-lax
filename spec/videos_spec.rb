...
describe 'videos' do 
  ...
  describe '#update_queue' do
    it 'adds videos to the end of your queue' do
      expect(update_queue(@videos, "Why can't girls code? They're beautiful.")).to eq ["PSY - GANGNAM STYLE", "Wiz Khalifa - See You Again ft. Charlie Puth", "Mark Ronson - Uptown Funk ft. Bruno Mars", "Taylor Swift - Blank Space", "Justin Bieber - Sorry", "How To Apply | Kode With Klossy", "Why can't girls code? They're beautiful."]
    end
  end

  describe '#view_queue' do
    it 'prints out the videos in the queue using the each method' do
      output = capture_stdout { view_queue(@videos) }
